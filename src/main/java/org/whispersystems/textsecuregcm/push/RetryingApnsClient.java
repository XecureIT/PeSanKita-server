...
public class RetryingApnsClient {
      ...
      throws IOException
  {
    ...
    this.apnsClient = new ApnsClientBuilder()
                      .setClientCredentials(new File(apnCertificate), apnKey)
                      .build();
  ...
  private static X509Certificate initializeCertificate(String pemCertificate) throws IOException {
    FileInputStream fis = new FileInputStream(pemCertificate);
  ...
  private static PrivateKey initializePrivateKey(String pemKey) throws IOException {
    FileInputStream fis = new FileInputStream(pemKey);
    PEMReader reader = new PEMReader(new InputStreamReader(fis));
